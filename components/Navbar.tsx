      ...
      <div className={`fixed top-0 left-0 right-0 z-[500] flex justify-center p-4 md:p-6 transition-transform duration-700 ${isOpen ? '-translate-y-24' : 'translate-y-0'}`}>
        <header
          className={`
            ...
            ${isScrolled
              ? 'w-full max-w-[900px] h-14 rounded-2xl bg-white/70 backdrop-blur-2xl border border-white/40 shadow-sm'
              : 'w-full max-w-[1340px] h-16 rounded-none bg-transparent'
            }
          `}
        >
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map(link => (
              <button key={link.id} onClick={() => handleNav(link.id)} className="text-[11px] font-black uppercase tracking-[0.2em] text-navy-custom/30 hover:text-navy-custom transition-colors">
                {link.label}
              </button>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => onNavigate('contact')} className="bg-navy-custom text-white px-5 py-2 rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-navy-custom/10">
              Contact
            </button>
